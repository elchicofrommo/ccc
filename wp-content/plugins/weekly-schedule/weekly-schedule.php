...
function ws_library(
	...
	foreach ( $daysoftheweek as $day ) {
		for ( $daysrow = 1; $daysrow <= $day->rows; $daysrow ++ ) {
			...
			// start new div line for 
			$output .= '<div class="scheduleDay">' . $day->name . '</div> ';
			$output .= '<div class="scheduleEvents"> ';

			if ( $items ) {
				// for each entry in the schedule
				foreach ( $items as $item ) {

					// output the ename and descritpion with any border color coming from the meeting category
					$output .= '  <div class="scheduleEvent" style="border-left-color:' . ( !empty( $item->itemcolor ) ? $item->itemcolor : $item->categorycolor ) . ';">';
					
					$output .= '    <div class="eventLabel">' . stripslashes($item->categoryname) . '</div>' ;
					$output .= '    <div class="eventValue">' . stripslashes( $item->itemname ) . '</div>' ;
					$output .= '    <div class="eventLabel">Time</div>';
					$output .= '    <div class="eventValue">' . stripslashes( $item->starttime ) . '-' . stripslashes( $item->endtime ) . '</div>' ;
					$output .= '   </div>';
					if( $item->descritpion ){
						$output .= '    <div class="eventDescription">' . stripslashes( $item->descritpion ) . '</div>' ;
				...
				$output .= '<span style= "width: 30px; background-color: red;" > </span>' 
					. '<span style="width: 600px;">No schedule data</span></br>';
			}
...
$genoptions['fullstylesheet'] = file_get_contents( $stylesheetlocation );
	...
	echo ' <!-- mario: stylesheet location is ' .  $stylesheetlocation . ' -->';
	echo ' <!-- mario: stylesheet contents ' .  $genoptions['fullstylesheet'] . ' -->';
	if ( $wsstylesheet ) {
